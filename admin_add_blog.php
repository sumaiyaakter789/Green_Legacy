...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Insert into database (admin posts are published directly)
    $stmt = $conn->prepare("INSERT INTO blogs 
        (title, content, author_id, tags, meta_description, featured_image, status, published_at, approved_by) 
        VALUES (?, ?, ?, ?, ?, ?, 'published', NOW(), ?)");
    $stmt->bind_param("ssisssi", $title, $content, $author_id, $tags, $meta_description, $image_path, $author_id);
    
    if ($stmt->execute()) {
        $_SESSION['success_msg'] = "Blog published successfully!";
        header("Location: admin_blogs.php");
        exit();
    } else {
        $_SESSION['error_msg'] = "Error publishing blog: " . $conn->error;
    }
}
?>
    ...
    <div class="ml-0 lg:ml-[280px] min-h-screen bg-gray-50">
        <div class="p-6">
            ...
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-3xl font-bold text-gray-800">Add New Blog Post - Admin Panel</h1>
