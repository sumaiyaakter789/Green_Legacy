...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image'])) {
        ...
        if ($check === false) {
            ...
            header("Location: admin_add_banner.php");
            exit;
        }
    ...
    $stmt = $conn->prepare("INSERT INTO banners (title, description, image_path, link, button_text, is_active, position, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssiiss", $title, $description, $image_path, $link, $button_text, $is_active, $position, $start_date, $end_date);
    $stmt->execute();
...
<head>
    ...
    <title>Add New Banner - Admin Panel</title>
...
<body>
    <div class="p-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold">Add New Banner</h1>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form method="POST" action="admin_add_banner.php" enctype="multipart/form-data">
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded"></textarea>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">Image * (Recommended: 1920x600px)</label>
                    <input type="file" name="image" required accept="image/*" class="w-full px-3 py-2 border border-gray-300 rounded">
                </div>

                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-gray-700 mb-2">Link URL (optional)</label>
                        <input type="url" name="link" class="w-full px-3 py-2 border border-gray-300 rounded">
                    </div>
                    <div>
                        <label class="block text-gray-700 mb-2">Button Text (optional)</label>
                        <input type="text" name="button_text" class="w-full px-3 py-2 border border-gray-300 rounded">
                    </div>
                </div>

                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">Position (order of display)</label>
                    <input type="number" name="position" value="0" class="w-full px-3 py-2 border border-gray-300 rounded">
                </div>
